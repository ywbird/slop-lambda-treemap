...
- [x] **내보낼 때 cellGap 만큼 외곽 패딩** — 화면 표시는 그대로 가장자리까지, 내보낼 때만 안쪽으로
- [x] **내보내기 크기 직접 지정** (가로×세로 px 입력, 기본 = 현재 캔버스)
- [x] **수동 색상** — 변수 이름별 HSL 슬라이더로 override (λ 테두리 + 해당 이름 변수 셀 모두 같은 색)
- [x] **보기 모드** — `세로` (기본) / `좌우` (입력 ↔ 트리맵) / `트리맵만` (입력 숨김, 상태/에러는 캔버스 위 오버레이)
- [ ] (선택) 호버 시 노드 정보 표시
