...
- [x] 노드별 렌더링 규칙 — 모든 AST 노드(변수/추상화/적용)가 트리맵의 노드로 대응
  - **변수(var)**: 고유 색으로 채운 사각형
    - 변수 이름 → 해시 → 색상(HSL 등) 매핑으로 unique color 보장
    - 같은 이름(또는 같은 바인딩)의 변수는 항상 같은 색
  - **추상화(lambda)**: 그 자체가 하나의 노드 셀(테두리만, 파라미터 색).
    body를 감싸는 래퍼가 아니라 body와 **형제로 분할**됨 (λ노드 | body)
  ...
  - 모든 분할(app, λ노드|body)에 동일한 공식의 셀 간 간격
- [x] 색상 안정성 고려
...
- [x] 입력: 람다 식 텍스트 입력 + 예시 프리셋 (예: `(λx. λy. x) a b`)
  - 프리셋: K 조합자 / 자기 적용 / 다중 대입 / 후속 숫자 / 오메가
